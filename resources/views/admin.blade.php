...
<body class="bg-wedding-gray">
    ...
    <nav class="bg-wedding-blue shadow-md">
        <div class="container mx-auto px-4 py-4 flex justify-between items-center">
            ...
            <a href="{{ url('/') }}" class="text-wedding-dark hover:text-wedding-pink transition">Voltar à Lista</a>
        </div>
    </nav>
    ...
    <div class="container mx-auto py-8">
        ...
        <div class="mb-6">
            <a href="{{ route('settings.edit') }}" class="text-wedding-blue hover:text-wedding-pink transition flex items-center">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 mb-8">
            ...
            <form action="{{ url('/admin/add') }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="name" id="name" class="border border-wedding-blue rounded-lg w-full px-4 py-3 focus:ring-2 focus:ring-wedding-pink focus:border-transparent" required>
                    @error('name')
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" class="border border-wedding-blue rounded-lg w-full px-4 py-3 focus:ring-2 focus:ring-wedding-pink focus:border-transparent"></textarea>
                    @error('description')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <div class="mb-6">
                    ...
                    <input type="url" name="purchase_link" id="purchase_link" class="border border-wedding-blue rounded-lg w-full px-4 py-3 focus:ring-2 focus:ring-wedding-pink focus:border-transparent" placeholder="https://exemplo.com/produto">
                    @error('purchase_link')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <button type="submit" class="bg-gradient-to-r from-wedding-blue to-wedding-pink text-white px-6 py-3 rounded-lg hover:from-wedding-pink hover:to-wedding-blue transition">Adicionar</button>
            </form>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
            ...
            <div class="overflow-x-auto">
                <table class="w-full table-auto border-collapse">
                    <thead>
                        <tr class="bg-wedding-blue text-wedding-dark">
                            <th class="border px-4 py-3">Nome</th>
                            <th class="border px-4 py-3">Descrição</th>
                            <th class="border px-4 py-3">Link de Compra</th>
                            <th class="border px-4 py-3">Selecionado Por</th>
                            <th class="border px-4 py-3">Telefone</th>
                            <th class="border px-4 py-3">Ações</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($presents as $present)
                            <tr class="hover:bg-wedding-gray transition">
                                <td class="border px-4 py-3">{{ $present->name }}</td>
                                <td class="border px-4 py-3">{{ $present->description ?? 'Sem descrição' }}</td>
                                <td class="border px-4 py-3">
                                    @if ($present->purchase_link)
                                        <a href="{{ $present->purchase_link }}" target="_blank" class="text-wedding-blue hover:text-wedding-pink transition">Link</a>
                                    @else
                                        Sem link
                                    @endif
                                </td>
                                <td class="border px-4 py-3">{{ $present->is_selected ? $present->selected_by : 'Não escolhido' }}</td>
                                <td class="border px-4 py-3">{{ $present->is_selected ? ($present->selected_by_phone ?? 'Não informado') : 'Não escolhido' }}</td>
                                <td class="border px-4 py-3 flex items-center space-x-2">
                                    ...
                                    @if ($present->is_selected)
                                        <form action="{{ route('presents.removeDonor', $present->id) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja liberar este presente?')">
                                            @csrf
                                            @method('PATCH')
                                            <x-secondary-button>Liberar</x-secondary-button>
                                        </form>
                                    @endif
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="border px-4 py-3 text-center text-gray-600">Nenhum presente cadastrado ainda.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
